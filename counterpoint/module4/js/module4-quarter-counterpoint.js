...
const I18N = {
  ...
  fr: {
    ...
    title: "Module 4 — Contrepoint à deux voix en noires",
...
function getRequiredQuarterCount() {
  ...
  // Module 4: four quarter-note slots per cantus note,
  // but the final counterpoint measure closes with one whole note.
  return Math.max(1, (cantusLength - 1) * SCORE.quartersPerCantus + 1);
}

function getPlaybackLength() {
  return getRequiredQuarterCount();
}
...
function playVerticalSonority(index) {
  ...
  const noteDuration = Math.max(0.42, qDuration * 1.08);
  ...
  if ((mode === "both" || mode === "cantus") && cantusNote) {
    const cantusDuration = index % SCORE.quartersPerCantus === 0 ? Math.max(1.05, qDuration * 4.25) : noteDuration;
    if (mode === "cantus" || index % SCORE.quartersPerCantus === 0) {
      playNoteName(cantusNote, cantusDuration, mode === "cantus" ? 1 : 0.74, "maleSample");
    }
  }

  if ((mode === "both" || mode === "counterpoint") && counterpointNote) {
    const isFinalCounterpointWhole = index === getRequiredQuarterCount() - 1;
    const counterpointDuration = isFinalCounterpointWhole ? Math.max(1.05, qDuration * 4.1) : noteDuration;
    playNoteName(counterpointNote, counterpointDuration, 1.05, "femaleSample");
  }
}
...
function drawStaff(svg, bottomLineY, label, noteCount, clefType = "treble") {
  ...
  // Invisible fallback geometry keeps the staff area stable for hit testing.
  for (let i = 0; i < 5; i++) {
    ...
    svg.appendChild(createSvgElement("line", {
      ...
      x2: endX,
      y2: y,
      class: "staff-line png-staff-fallback-line"
    }));
  }
...
function drawAccidental(svg, parsed, x, y, isCantus, isSelected, isCurrentPlayback, issueClass = "") {
  ...
  svg.appendChild(createSvgImage(
    ...
    width,
    height,
    ["png-notation", "png-accidental", parsed.accidental === "#" ? "sharp" : "flat", isCantus ? "cantus" : "", isSelected ? "selected" : "", isCurrentPlayback ? "playing" : "", issueClass].filter(Boolean).join(" ")
  ));
}

function drawQuarterFlag(svg, x, y, isSelected, isCurrentPlayback) {
  // Module 4 uses quarter notes. Quarter notes have no flag.
  return;
}
...
function renderScore() {
  ...
  const cantus = getNotesFromTextarea("cantus");
  const requiredQuarterCount = getRequiredQuarterCount();
  const counterpoint = getNotesFromTextarea("counterpoint").slice(0, requiredQuarterCount);
  const quarterCount = Math.max(requiredQuarterCount, 1);
...
function loadSelectedExercise() {
  ...
  const required = Math.max(1, ((exercise.cantus || []).length - 1) * SCORE.quartersPerCantus + 1);
...
function setExample() {
  const select = document.getElementById("exerciseSelect");
  if (select) {
    select.value = "module4-example-filled";
    updateExerciseDescription();
  }
...
function octaveForLetterInput(letter) {
  return ["C", "D", "E", "F", "G", "A", "B"].includes(letter) ? 4 : 4;
}

function inputLetterNote(letter) {
  if (isPlaying) return;
  const normalized = String(letter || "").toUpperCase();
  if (!["A", "B", "C", "D", "E", "F", "G"].includes(normalized)) return;
  ...
  renderScore();
  updateDisplays();
  playNoteName(note, 0.65, 1.05, "femaleSample");
